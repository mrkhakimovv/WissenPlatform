...
const startIdx = code.indexOf('  return createPortal(');
const headerEndIdx = code.indexOf('<div className="flex flex-1 overflow-hidden min-w-0">');

if (startIdx === -1 || headerEndIdx === -1) {
  console.error('Could not find boundaries');
  process.exit(1);
}

const before = code.substring(0, startIdx);
let rest = code.substring(headerEndIdx + '<div className="flex flex-1 overflow-hidden min-w-0">'.length);

const endIdx = rest.lastIndexOf('document.body');
if (endIdx === -1) {
  console.error('Could not find end document.body');
  process.exit(1);
}

let center = rest.substring(0, rest.lastIndexOf('</div>', endIdx));
let after = rest.substring(rest.lastIndexOf('</div>', endIdx) + 6); // past that last div

// Actually the structure at the end of the file is:
//       </div>
//     </div>,
//     document.body
//   );
...
// Let's just find the last </div> before document.body
const newCode = before + returnStatementNew + center + '      </div>\n      )}\n    </div>,\n    document.body\n  );\n}';

fs.writeFileSync('src/pages/admin/AdminCertificateBuilder.tsx', newCode);

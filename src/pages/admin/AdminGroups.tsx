  ...
  return (
    <div className="flex flex-col h-full space-y-4 pb-6">
      <div className="flex items-center justify-between">
        <h2 className="text-white font-bold text-lg">Guruhlar</h2>
        <button 
          onClick={() => setIsGroupModalOpen(true)}
          className="bg-[#FEC204] text-black w-9 h-9 rounded-full flex items-center justify-center hover:scale-105 active:scale-95 transition-transform shadow-lg shadow-[#FEC204]/20"
        >
          <Plus size={20} />
        </button>
      </div>
      ...
      <div className="relative shrink-0">
        ...
        <input 
          ...
          className="w-full glass-panel py-3 pl-11 pr-4 focus:outline-none focus:border-[#FEC204]/40 text-white placeholder-white/25 text-sm"
        />
      </div>

      <div className="overflow-y-auto space-y-3 flex-1 pb-4">
        {filtered.length === 0 && (
          <p className="text-center text-white/30 py-10 text-sm">Guruhlar topilmadi</p>
        )}
        ...
        {filtered.map((g, i) => (
          ...
          >
            <div className="flex items-start justify-between">
              <div>
                <h3 className="font-bold text-white text-base leading-tight mb-1">{g.name}</h3>
            ...
            >
              ...
              <form onSubmit={handleAddGroup} className="space-y-4">
                <div className="space-y-1">
                  <label className="text-[11px] text-white/50 px-1 uppercase tracking-wider font-bold">Guruh nomi</label>
                ...
                <div className="space-y-1">
                  ...
                  <select required value={subject} onChange={e=>setSubject(e.target.value)} className="w-full glass-panel p-3 outline-none focus:border-[#FEC204]/50 text-sm bg-transparent appearance-none">
                    <option value="" disabled className="text-black">Fanni tanlang...</option>
                    {subjects.map(s => (
                ...
                <div className="space-y-1">
                  <label className="text-[11px] text-white/50 px-1 uppercase tracking-wider font-bold">O'qituvchi ism familiyasi</label>
                  <select required value={teacherName} onChange={e=>setTeacherName(e.target.value)} className="w-full glass-panel p-3 outline-none focus:border-[#FEC204]/50 text-sm bg-transparent appearance-none">
                    <option value="" disabled className="text-black">O'qituvchini tanlang...</option>
                    {teachers.map(t => (
                      <option key={t.id} value={t.fullName} className="text-black">{t.fullName}</option>
                    ))}
                  </select>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1 col-span-2">
                    <label className="text-[11px] text-white/50 px-1 uppercase tracking-wider font-bold">Dars soati</label>
                    <div className="flex gap-2">
                      <input required type="time" value={startTime} onChange={e=>setStartTime(e.target.value)} className="w-full glass-panel p-3 outline-none focus:border-[#FEC204]/50 text-sm text-white/90" style={{ colorScheme: "dark" }} />
                      <input required type="time" value={endTime} onChange={e=>setEndTime(e.target.value)} className="w-full glass-panel p-3 outline-none focus:border-[#FEC204]/50 text-sm text-white/90" style={{ colorScheme: "dark" }} />
                    </div>
                  </div>
                  <div className="space-y-1 col-span-2">
                    <label className="text-[11px] text-white/50 px-1 uppercase tracking-wider font-bold mb-1 block">Kunlar</label>
                ...
                <div className="flex gap-3 pt-2 mt-4">
                  <button type="button" disabled={loading} onClick={()=>setIsGroupModalOpen(false)} className="flex-1 py-3 rounded-xl border border-white/10 text-sm font-medium hover:bg-white/5 transition-colors text-white/70">Bekor qilish</button>

...
let insideReturn = code.substring(startIdx);

insideReturn = insideReturn.replace(`  return createPortal(
    <div className="fixed inset-0 bg-[#0d0d0d] z-[9999] flex flex-col animate-in fade-in zoom-in-95 duration-200">
      {/* Header */}
      <div className="flex items-center justify-between p-4 border-b border-white/10 shrink-0 bg-[#121212]">
        <div>
      ...
      <div className="flex flex-1 overflow-hidden min-w-0">`, returnStatementNew);

// Add the closing for `isFastMode ? (...) : (`
insideReturn = insideReturn.replace(/      <\/div>\s*<\/div>\s*\);\s*}\s*$/, `      </div>
      )}
    </div>
  );
}
...
fs.writeFileSync('src/pages/admin/AdminCertificateBuilder.tsx', beforeReturn + insideReturn);
